...
taxo <- read_tsv('W:/ninon-species/output/Output_M2/AV_AP_ARG/Dataframe/sliced_all_species_taxo.tsv', col_types = "cccccccc") %>% 
  as.data.frame()
...
taxo %>% 
  ...
  #arrange(pident, qcovhsp) %>% # Pour les ARGs uniquement
...
level <- as.data.frame(taxo_small[, c(8:13)])
...
level_name <- unlist(colnames(taxo_small[, c(2:7)]))
