...
#### Join de l'abre avec les familles de la table de taxonomie (== remplacer les labels par les familles associées) ####
taxo_tree <- left_join(tree_df, small_taxo, by = c('label' = 'sseqid')) # On join sur les labels de séquences bactériennes
taxo_tree[1:Ntip(tree), 'label'] <- taxo_tree[1:Ntip(tree), 'Family']  # On remplace les labels de tips (pas de nodes) par les familles associées 
...
#### Join de l'abre avec les familles de notre dataframe (== pruner l'arbre en ne gardant que les familles présentes dans notre dataframe) ####
...
na_label <- is.na(phylo_tree[1:n_tips, 'family_shared_by']) # On extrait les labels des tips matchés lors du join
...
na_label <- is.na(phylo_tree[1:Ntip(new_tree), 'family_shared_by']) # On extrait les labels des tips non-matchés
na_family <- which(na_label == TRUE)

next_tree <- as.phylo(phylo_tree) # On passe au format phylo pour pouvoir pruner l'arbre
next_tree <- drop.tip(next_tree, na_family) # On prune l'arbre en suppriment les tips non-matchés
...
#### Enregistrement de l'arbre au format 'phylo' dans le fichier Family_tree.tree (même résultat qu'on parte de Taxo_result.tsv ou de New_Taxo_result.tsv) ####
write.tree(next_tree, "W:/ninon-species/output/Family_tree.tree")