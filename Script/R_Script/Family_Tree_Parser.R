...
#### Prétraitement des données en vue de la création d'une matrice d'absence/présence CentroidexFamille ####
...
#### Créaction d'une matrice binaire (0/1) d'absence/présence des centroides au niveau des familles ####
...
splot <- ggplot(family_length, aes(length)) + geom_histogram(bins = n_centro)
splot + ggtitle("Distance inter-familles")
...
ftree <- ggtree(tree) 
ftree + ggtitle("arbres des familles par centroides")

ftree <- ggtree(tree) + geom_hilight(data = liste, mapping = aes(node = node, fill = type))
ftree + ggtitle("sous-arbres des familles par centroides")
