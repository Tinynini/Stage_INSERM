...
Ce github vous donne accès à l'ensemble des scripts R et bash (hors période d'entrainement) que j'ai codé lors de mes stages de M1 et de M2. La numérotation des scripts R permet de savoir dans quel ordre ils doivent être utilisés, les scripts 00-main.R des 3 jeux de spripts de M2 permettant de lancer directement les autres scripts du même jeu. Tous les plots obtenus cette année avec le dataset de M1 ('ARG') et avec le dataset de M2 ('AV_AP_ARG') sont également disponibles dans la partie 'Plot'.

En revanche, par respect de la confidentialité qu'implique toute recherche scientifique, les données de départ et les fichers d'output n'y figurent pas. Vous ne pourrez donc pas tester les codes par vous-même. Les scripts sont duement légendés pour compenser cela. 
